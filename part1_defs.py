...
def chi_squared(L, data):
    #takes model L value in erg/sA and observed m, err and z data and runs chi-squared minimisation and returns min chi2 and chired
    z = data[:,0]
    m_obs = data[:,1]
    err = data[:,2]
    ...
    p = 1           # number of fitted parameters (here L)
    chi2_red = chi2 /len(m_obs)
