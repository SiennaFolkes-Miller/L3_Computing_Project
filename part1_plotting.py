...
inset.tick_params(axis='x', labelsize=12) 
...
ax2.hist(residuals, bins=5, color='magenta', orientation='horizontal')
...
ax.set_xlabel('Peak luminosity', fontsize=14, fontweight='bold', fontname='Arial')
...
ax.set_xticks([np.log(best_L.value),74.5,74.6,74.7,74.8,74.9,75.0])
ax.set_xticklabels([r'$\mathbf{Best\ L_{peak}}$','74.5','74.6','74.7','74.8','74.9','75.0'])
ax.set_ylim(20,35)
ax.set_yticks([chi2_min, chi2_min+1,20,23,26,29,32,35])
ax.set_yticklabels(['$\\chi^2_{min}$','$\\chi^2_{min}+1$','20','23','26','29','32','35'])
textstr = (r"$\mathbf{\chi^2_{min}} = \mathbf{" + f"{chi2_min:.2f}" + r"}$" + "\n" r"$\mathbf{\chi^2_{red}} = \mathbf{" + f"{chi2_red:.2f}" + r"}$")
ax.text(0.05, 0.4, textstr,transform=ax0.transAxes,fontsize=16,verticalalignment='top',horizontalalignment='left',alpha=0.9)
plt.show()