...
z_model = np.linspace(min(z_obs)*0.9, max(z_obs)*1.1, 200)
m_model_vals = m_model(z_model, best_L)
ax0.plot(z_model, m_model_vals, color='red', linewidth = 3, label='Best-fit model')
ax0.errorbar(z_obs, m_obs, yerr=err, color = 'blue', fmt='o', label='Observed', capsize=8)
ax0.set_ylabel('Effective Magnitude (m)',fontsize=20, fontweight='bold', fontname='Arial')
ax0.tick_params(labelbottom=False, labelsize=14) 
textstr = (r"$\mathbf{\chi^2_{min}} = \mathbf{" + f"{chi2_min:.2f}" + r"}$" + "\n" r"$\mathbf{\chi^2_{red}} = \mathbf{" + f"{chi2_red:.2f}" + r"}$")
ax0.text(0.05, 0.95, textstr,transform=ax0.transAxes,fontsize=20,verticalalignment='top',horizontalalignment='left',alpha=0.9)

#full data and model fit inset
inset = inset_axes(ax0, width="50%", height="50%", loc='lower right', borderpad=1)
inset.plot(z_model, m_model_vals, color='red', linewidth = 3)
inset.errorbar(z_obs, m_obs, yerr=err, color = 'blue', fmt='o', markersize=5)
z_model2 = np.linspace(max(z_obs)*1.1, max(z_obs2)*1.1, 200)
m_model_vals2 = m_model(z_model2, best_L)
inset.plot(z_model2, m_model_vals2, color='red', linestyle = '--', linewidth =3)
inset.errorbar(z_obs2, m_obs2, yerr=err2, color = 'darkblue', fmt='o', markersize=5)
x_mark = 0.1
inset.axvline(x=x_mark, color='grey', linestyle='-', linewidth=3)
inset.set_xticks([])  
inset.set_yticks([])  
inset.set_xticks([0.1])
inset.set_xticklabels([r'$\mathbf{z=0.1}$'])
inset.tick_params(axis='x', labelsize=16) 
inset.xaxis.set_label_position('top') 
inset.xaxis.tick_top() 
...
ax1.axhline(y=-1, color='grey', linestyle='--')
ax1.scatter(z_obs, residuals, color='magenta')
ax1.set_xlabel('Redshift (z)',fontsize=20, fontweight='bold', fontname='Arial')
ax1.set_ylabel('Normalised \n Residuals',fontsize=16, fontname='Arial')
ax1.xaxis.set_tick_params(labelsize=14)
ax1.set_yticks([-1,0,1])
ax1.set_yticklabels([r'$-1\sigma$', r'$0$', r'$1\sigma$'])
ax1.yaxis.set_tick_params(labelsize=14)

#histogram of residuals
ax2 = fig.add_subplot(gs[1, 1], sharey=ax1) 
ax2.hist(residuals, bins=6, color='magenta', orientation='horizontal', density=True)
ax2.axhline(y=0, color='black')
ax2.axhline(y=1, color='grey', linestyle='--')
ax2.axhline(y=-1, color='grey', linestyle='--')
y = np.linspace(min(residuals), max(residuals), 300)
...
ax2.plot(pdf_perf, y, color='grey', lw=2)
ax2.plot(pdf_resid, y, color='purple', lw=3)
ax2.set_xlabel('Probability Density',fontsize=16, fontname='Arial')
ax2.tick_params(left=False, labelleft=False, labelsize=14)  
fig.savefig("Poster_L_plot.png", dpi=300, bbox_inches='tight')
plt.show()